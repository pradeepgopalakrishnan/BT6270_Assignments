...
% The peaks above a certain threshold(20 mV) are detected and 
% an average frequency is calculated with the individual frequencies.
% The plots have been commented out.

[pks loc]=findpeaks(vhist, "DoubleSided");
valid_loc=[];
for i=1:length(pks)
  if pks(i)>20
    valid_loc=[valid_loc loc(i)];
  end;
end;

valid_loc=sort(loc);
valid_loc_shift=shift(valid_loc, 1);
valid_loc_final=valid_loc-valid_loc_shift;
valid_loc_final=valid_loc_final(2:end);
freq=1./valid_loc_final;

disp(ImpCur);
avg_freq=sum(freq)/length(freq);
disp(avg_freq);
